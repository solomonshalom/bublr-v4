...
export default function Profile({ user }) {
  useEffect(() => {
    ...
    // Only load animations if user has a link to animate
    if (!user.link) return;
    
    // Delay animation loading to not block initial render
    const timeoutId = setTimeout(() => {
      if (!mounted) return;
      
      const loadDrawSVGPlugin = async () => {
        try {
          // Load DrawSVG plugin dynamically only when needed
          ...
          // Simplified animation with fewer variants
          const svgVariants = [
...
export async function getServerSideProps({ params }) {
  ...
  try {
    ...
    const processedPosts = user.posts
      .filter(p => p.published) // Filter first to reduce processing
      .map(p => ({
        ...p,
        lastEdited: p.lastEdited?.toDate?.() ? p.lastEdited.toDate().getTime() : p.lastEdited?.toMillis?.() || Date.now(),
      }))
      .sort((a, b) => b.lastEdited - a.lastEdited) // Sort by most recent
